...
>(target: T, k: K, detail: D) {
  target.dispatchEvent(new CustomEvent(k, { detail }));
}
...
export const WithEvents = <EM extends ValueIsEvent<EM>, T>(target: {new(...args: any): T}): typeof target & {new(...args: any): T & TypedEventTarget<EM>} => target as any;
export type WithEvents<EM extends ValueIsEvent<EM>, T extends {new(...args: any): EventTarget}> = T & {new(...args: any): InstanceType<T> & TypedEventTarget<EM>};
export const SafeTarget: {new(...args: any): Omit<EventTarget, keyof TypedEventTarget<{}>>} = EventTarget;